...
public class Main {
    ...
    private static Fleet buildFleet() {
        ...
        fleet.add(new Car("1HGCM82633A004352", "Honda", "Accord", 2023,
                "Blue", 4, 2.0, FuelType.GASOLINE, 15.8, 4));
        fleet.add(new Car("5YJ3E1EA7PF123456", "Tesla", "Model 3", 2024,
                "Red", 4, 0.0, FuelType.ELECTRIC, 75.0, 4));
        fleet.add(new Car("JTDKARFU2J3061234", "Toyota", "Prius", 2020,
                "Silver", 4, 1.8, FuelType.HYBRID, 11.3, 5));
        fleet.add(new Truck("1FT8W3BT5MEC12345", "Ford", "F-350", 2021,
                "White", 6, 6.7, FuelType.DIESEL, 40.0, 3500.0));
        fleet.add(new Truck("3C6UR5DL9JG123456", "Ram", "2500", 2019,
    ...
    private static void printReport(Fleet fleet) {
        ...
        System.out.printf("%-20s: %.1f L%n", "Average engine size",
                fleet.averageEngineSize());
        System.out.printf("%-20s: %d %s %s (%.1f mi)%n", "Longest range",
                longest.getYear(), longest.getMake(), longest.getModel(),
                longest.rangeInMiles());
        for (FuelType fuel : FuelType.values()) {
            System.out.printf("  %-9s: %d%n", fuel.getLabel(),
    ...
    private static void printGuardRails(Fleet fleet) {
        Vehicle accord = fleet.findByVin("1HGCM82633A004352");
        System.out.printf("%-23s: %s%n", "Duplicate add", fleet.add(accord));
        System.out.printf("%-23s: %s%n", "Remove Prius",
                fleet.removeByVin("JTDKARFU2J3061234"));
        System.out.printf("%-23s: %s%n", "Fleet size", fleet.size());

        try {
            new Car("1HGCM82633A004352", "Honda", "Accord", 2023,
                    "Blue", 4, 2.0, FuelType.ELECTRIC, 15.8, 4);
